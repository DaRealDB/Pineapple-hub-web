...
/**
 * Format OCR confidence score for display.
...
/**
 * Format OCR processing time for display.
 * @param {number} ms - Processing time in milliseconds
 * @returns {string} e.g. "125ms"
 */
...
export function getConfidenceLevel(confidence) {
  if (confidence == null) return { label: 'UNKNOWN', color: 'text-tertiary' };
  if (confidence >= 0.8) return { label: 'HIGH', color: 'text-[#10B981]' };
  if (confidence >= 0.5) return { label: 'MEDIUM', color: 'text-[#F59E0B]' };
  return { label: 'LOW', color: 'text-[#EF4444]' };
}
